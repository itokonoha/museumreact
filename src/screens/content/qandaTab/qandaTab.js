...
import { getQuestionsData, loadMoreQuestions, pullRefreshQuestions } from './QuestionsActions';


const mapStateToProps = ({ QuestionsReducer, LoginReducer, SignUpReducer }) => {
  return {
    questionsData: QuestionsReducer.questionsData,
    loading: QuestionsReducer.loading,
    loadMore: QuestionsReducer.loadMore,
    auth: SignUpReducer.auth,
    login: SignUpReducer.login,
    pullToRefresh: QuestionsReducer.pullToRefresh,
  };
};
class QuestionsTab extends Component {
  constructor(props) {
    ...
    if (this.props.questionsData === null || this.props.questionsData.length === 0 ) {
        ...
        try {
          const value = AsyncStorage.getItem('user_token',(err, result) => {
              ...
              if (result !== null && result !== undefined && result !== '') {
                ...
                this.props.getQuestionsData(this.state.page, '');
              }
  ...
  componentDidMount(){
    if (this.props.questionsData.length === 0) {
      this.setState({nodata:'No questions available .'},() => console.log(this.state.nodata));
    }
  }
  ...
  _onRefresh() {
      this.props.pullRefreshQuestions();
      try {
        const value = AsyncStorage.getItem('user_token',(err, result) => {
            if (result !== null && result !== undefined && result !== '') {
              console.log("TOKEN",result);
              this.setState({ page: 1}, ()=>this.props.getQuestionsData(this.state.page, result) );

            }else {
              this.setState({ page: 1}, ()=>this.props.getQuestionsData(this.state.page, '') );
            }
...
export default connect(mapStateToProps, { getQuestionsData, loadMoreQuestions, pullRefreshQuestions })(QuestionsTab);
